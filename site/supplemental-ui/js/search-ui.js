(function (global, factory) {
  ...
  const searchLabel = document.getElementById('search-label');
   if(searchLabel) {
    ...
    if(title) {
      searchLabel.textContent = "Search " + title + ": ";
    }
  }
  ...
  function filter (result, documents) {
    const module = document.querySelector('meta[name="component"]').content
    if(searchModule.checked && module){
      return result.filter((item) => {
        const ids = item.ref.split('-');
        const docId = ids[0];
        const doc = documents[docId];
        return doc.component == module
      })
    }
    return result
  }
  ...
  function initSearch (lunr, data) {
    ...
    if (searchModule) {
      searchModule.parentElement.addEventListener('click', confineEvent);
      searchModule.addEventListener('change', (e) => toggleFilter(e, index));
    }
